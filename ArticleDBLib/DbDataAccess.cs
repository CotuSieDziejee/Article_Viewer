...
namespace ArticleDBLib
{
    public class DbDataAccess
    {
        ...
        //stworzony został interfejt w celu możliwości stworzenia listy, któa pomieści dwa typy danych
        //UWAGA!!! NIE JESTEM PEWIEN, CZY TO DZIAŁA!
        public static IGroupAbyK SortArticlesByKeywords() {
            string query = "select ar.*, kw.keyword from articles ar join keywordstoarticles kta on kta.id_article = ar.id join keywords kw on kw.id = kta.id_keyword order by keyword asc";
            IEnumerable<IGroupAbyK> output = Enumerable.Empty<IGroupAbyK>();
            try
            {
                using (SQLiteConnection cnn = new SQLiteConnection(LoadConnectionString()))
                { 
                    output = cnn.Query<IGroupAbyK>(query);
                }
            }
            catch (SQLiteException)
            {
            ...
            return (IGroupAbyK)output.ToList();
        }
    }
}
